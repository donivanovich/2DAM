...
#End Region
    ...
    Private Sub Form1_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ' **PASO 1**: Conecta a base de datos Access (.mdb)
        oConexion.ConnectionString = "Provider=Microsoft.Jet.OLEDB.4.0; Data Source=C:\Users\kosoi\Documents\GitHub\2DAM\vb\Databases\Db02_GeneradorConsultas\DatosPer.mdb"
        oConexion.Open()

        ' **PASO 2**: Carga tabla "Emple" en DataSet
        ...
        ' **PASO 3**: Llena ComboBox con nombres de tablas
    ...
    Private Sub CmbTablas_SelectedIndexChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles CmbTablas.SelectedIndexChanged
        ' **Cuando eliges TABLA**:
        CmbCampos.Items.Clear() ' Limpia campos anteriores

        Dim oDataRow As DataRow
        oDataRow = oDataSet.Tables(CmbTablas.Text).Rows(0) ' Primera fila de tabla
        MiTablas = oDataRow.Table ' Tabla actual

        ' Llena ComboBox con NOMBRES de CAMPOS (columnas)
    ...
    Private Sub CmbCampos_SelectedIndexChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles CmbCampos.SelectedIndexChanged
        ' **Cuando eliges CAMPO**:
        TxtSelect.Text = CadenaSelect + " Where " + CmbCampos.Text ' "select * from Emple Where Nombre"
        cadenaSelect1 = TxtSelect.Text

        Dim TypoDatoString As String = "System.String" ' Texto
        Dim TypoDatoInt As String = "System.Int32" ' Número

        ' **Detecta TIPO DE DATO del campo**
        For Each DatosCol In MiTablas.Columns
            If DatosCol.ColumnName = CmbCampos.Text Then
                Select Case DatosCol.DataType.FullName
                    Case TypoDatoString ' ← CAMPO es TEXTO
                        OptIgual.Enabled = True
                        OptMayor.Enabled = False ' Solo = para texto
                        OptMenor.Enabled = False
                        Sw = 0
                        Exit For
                    Case TypoDatoInt ' ← CAMPO es NÚMERO
                        OptIgual.Enabled = True
                        OptMayor.Enabled = True ' =, >, < para números
    ...
    Private Sub OptIgual_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles OptIgual.Click
        If OptIgual.Checked = True Then
            ActivarDato(OptIgual.Text) ' Llama función con "="
        End If
    End Sub
    ...
    Private Sub OptMayor_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles OptMayor.Click
        If OptMayor.Checked = True Then
            ActivarDato(OptMayor.Text) ' Llama con ">"
        End If
    End Sub
    ...
    Private Sub OptMenor_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles OptMenor.Click
        If OptMenor.Checked = True Then
            ActivarDato(OptMenor.Text) ' Llama con "<"
        End If
    End Sub

    Private Sub ActivarDato(ByVal Operador As String)
        ' **Activa campos de búsqueda**
        CambiarEstado(True) ' Enciende TxtDatoBuscar + BtnAceptar
        cadenaselect2 = cadenaSelect1 + " " + Operador ' "select * from Emple Where Nombre ="
        TxtSelect.Text = cadenaselect2
    End Sub

    Private Sub BtnAceptar_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles BtnAceptar.Click
        ' **¡BOTÓN PRINCIPAL! Ejecuta la búsqueda**
        BtnAceptar.Enabled = False ' Desactiva para evitar doble clic

        ' **Agrega el VALOR a buscar al SQL**
        Select Case Sw
            Case 0 ' ← Campo TEXTO: agrega comillas
                TxtSelect.Text = TxtSelect.Text + " '"
                TxtSelect.Text = TxtSelect.Text + TxtDatoBuscar.Text
                TxtSelect.Text = TxtSelect.Text + "'"
                ' Resultado: "select * from Emple Where Nombre = 'Juan'"
            Case 1 ' ← Campo NÚMERO: sin comillas
                TxtSelect.Text = TxtSelect.Text + " " + TxtDatoBuscar.Text
                ' Resultado: "select * from Emple Where Edad > 25"
        End Select

        ' **EJECUTA la consulta SQL**
        oConexion.Open()
        oDataAdapter = Nothing
        oDataAdapter = New OleDbDataAdapter(TxtSelect.Text, oConexion) ' Nueva consulta

        oDataSet.Tables("Emple").Clear() ' Borra datos anteriores
        oDataAdapter.Fill(oDataSet, "Emple") ' ¡LLENA con RESULTADOS!
        oConexion.Close()
        ...
        If oDataSet.Tables("Emple").Rows.Count > 0 Then
            ...
            DtgDatos.DataMember = "Emple" ' DataGrid se llena
        Else
            LblNoDatos.Visible = True
        End If

        CambiarEstado(False) ' Desactiva búsqueda hasta nueva selección
    End Sub

    Private Sub CambiarEstado(ByVal Valor As Boolean)
        ' **Enciende/Apaga campos de búsqueda**
